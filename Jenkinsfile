pipeline {
  ...
  environment {
    ...
    EXT_PIP = 'none'
    EXT_BLOB = 'none'
    BUILD_VERSION_ARG = 'KODI_RELEASE'
    LS_USER = 'thelamer'
    LS_REPO = 'docker-kodi-headless'
    DOCKERHUB_IMAGE = 'lspipelive/kodi-headless'
    DEV_DOCKERHUB_IMAGE = 'lspipetest/kodi-headless'
    PR_DOCKERHUB_IMAGE = 'lspipepr/kodi-headless'
    BUILDS_DISCORD = credentials('build_webhook_url')
    GITHUB_TOKEN = credentials('github_token')
    DIST_IMAGE = 'ubuntu'
    DIST_TAG = 'xenial'
    DIST_PACKAGES = 'libcdio13 \
  	                 libcurl3 \
  	                 libegl1-mesa \
  	                 libfreetype6 \
  	                 libfribidi0 \
  ...
  stages {
    ...
    stage("Set ENV github_devel"){
      when {
        expression {
          env.EXT_RELEASE_TYPE == 'github_devel'
        }
      }
      steps{
        script{
    ...
    stage('Github-Tag-Push-Release') {
      ...
      steps {
        ...
        sh '''#! /bin/bash
              if [ ${EXT_RELEASE_TYPE} == 'github_stable' ]; then
              ...
              elif [ ${EXT_RELEASE_TYPE} == 'github_devel' ]; then
                # Grabbing the current release body from external repo
